document.addEventListener('DOMContentLoaded', () => {
  ...
  async function openBrokerModal({ mc }) {
    ...
    try {
      //Ovo je poslednji deo koda koji sam uradio za v0725.alpha1 07/29/25 @ 04.57AM - Malo testiram kod i idem da spavam
      const res = await fetch('php/search.php?mc=' + encodeURIComponent(mc) + '&mode=edit');
      const data = await res.json();
      const broker = data.broker || (data.results && data.results[0]);
      
      if (!broker) {
        console.warn('Broker data not found');
        brokerModal.show();
        return;
      }
